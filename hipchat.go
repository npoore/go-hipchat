...
import (
	...
	"github.com/npoore/go-hipchat/xmpp"
)
	...
	for {
		...
		case "iq" + xmpp.NsJabberClient: // rooms and rosters
			...
			case xmpp.NsIqRoster:
				...
				if strings.Compare(attr["type"], "result") == 0 {
					c.receivedUsers <- items
				} else if strings.Compare(attr["type"], "set") == 0 { //{{jabber:client iq} [{{ to} 122816_4464859@chat.hipchat.com/bot} {{ type} set}]}
					fmt.Println(query)
				}
